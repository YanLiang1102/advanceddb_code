...
--driver.py will call task3 by complie in in jython
...
oldrank = LOAD '$doc_in' USING PigStorage(',') AS (tid:int,rank:float);
...
DUMP final7;
--rm the old fileSTORE final7 INTO '$doc_out' using PigStorage('\,','-schema');
STORE final7 INTO '$doc_out' using PigStorage(',','-schema');
